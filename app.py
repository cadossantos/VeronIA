...
def pagina_principal():
    """Controla e renderiza a interface principal de chat.

    Esta é a função central da UI. Ela verifica o estado da aplicação
    (`st.session_state`) para decidir o que mostrar:
    -   Se nenhum modelo foi carregado, exibe instruções para o usuário.
    -   Se nenhuma conversa foi selecionada, exibe um aviso.
    -   Se tudo estiver pronto, ela renderiza o histórico da conversa ativa.

    Ela também gerencia o input do usuário, o envio da mensagem para o modelo,
    a exibição da resposta em tempo real (stream) e a persistência dos dados
    (salvando a troca de mensagens no banco e na memória).
    """
